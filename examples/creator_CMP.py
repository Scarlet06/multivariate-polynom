...
#   -> 0
cm0:CMulty_X = ComplexMultyPolynomial.zeroMulty("x")
#   -> 1
cm1:CMulty_XY = ComplexMultyPolynomial.oneMulty("x","y")

#   -> 4*y^3 - 0.3333*x^5*y^2
cm_dict:CMulty_XY = ComplexMultyPolynomial({'0-3':1+2j,'5-2':-1/3}, ('x','y'))
#   -> 2*x + 2*C0
cm_dict_integrals:CMulty_X = ComplexMultyPolynomial({'1':2j},('x',),{'0':['C0',2]})

#   -> 3*x^2 + y + 1
cm_text:CMulty_XY = ComplexMultyPolynomial.fromText("(3j-4-7.7j+2)*3*x^2+1+y")
#   -> x^5*y^7
cm_text_unkn:CMulty_XYZ = ComplexMultyPolynomial.fromText("x^3*y^7*x^2+(-2j)",('z',))
#   -> 3*x*y - 7 + C*x
cm_text_integ:CMulty_XYZ = ComplexMultyPolynomial.fromText("-7+3*x*y+(1)*z^2",('y','x','z'),{'0-1-0':['C',1]})

#   -> eg: x^2 - 2x, max deg is 2
cm_random:CMulty_X = ComplexMultyPolynomial.random(2)
#   -> eg: -3*x^3 + 4*y*x - 2, max deg is 3
cm_random_unkn:CMulty_XY = ComplexMultyPolynomial.random(3,"y","x")
#   -> eg: -x^2*y + 4*y*x, max deg is 4 but y can have max power 1
cm_random_dict:CMulty_XY = ComplexMultyPolynomial.random(4,"x",y=1)
