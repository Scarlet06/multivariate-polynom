...
#   -> 2*x + 2*C0
m_dict_integrals = Multy_X({'1':2},('x',),{'0':['C0',2]})

#   -> 3*x^2 + y + 1
m_text = Multy_XY.fromText("3*x^2+1+y")
#   -> x^5*y^7
m_text_unkn = Multy_XYZ.fromText("x^3*y^7*x^2",('z',))
#   -> 3*x*y - 7 + C*x
m_text_integ = Multy_XYZ.fromText("-7+3*x*y",('y','x','z'),{'0-1-0':['C',1]})

#   -> eg: x^2 - 2x, max deg is 2
m_random = Multy_X.random(2)
#   -> eg: -3*x^3 + 4*y*x - 2, max deg is 3
m_random_unkn = Multy_XY.random(3,"y","x")
#   -> eg: -x^2*y + 4*y*x, max deg is 4 but y can have max power 1
m_random_dict = Multy_XY.random(4,"x",y=1)
